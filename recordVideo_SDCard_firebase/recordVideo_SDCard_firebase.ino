...
//________Include Firebase Lib_____________
#include <WiFi.h>
#include <FirebaseESP32.h>

// Cấu hình WiFi
#define WIFI_SSID "eoh.io"
#define WIFI_PASSWORD "Eoh@2021"

// Cấu hình Firebase
#define API_KEY "AIzaSyXXXXXXXXXXXXXXXXXXXXXX"          // Điền Web API Key từ Project Settings
#define FIREBASE_PROJECT_ID "esp32cam-4dbf9"            // Project ID từ hình của bạn
#define STORAGE_BUCKET_ID "esp32cam-4dbf9.appspot.com"  // Storage bucket = Project ID + .appspot.com

// Khởi tạo Firebase Data object
FirebaseData fbdo;
FirebaseAuth auth;
...
bool uploadVideo(const char *filename) {
  if (Firebase.ready()) {
    Serial.print("Uploading video file: ");
    ...
    String destination = "videos/" + String(millis()) + "_" + String(filename);

    // Configure upload parameters
    Serial.println("Preparing to upload...");
    if (Firebase.Storage.upload(&fbdo, STORAGE_BUCKET, filename,
                                "video/mp4",  // mime type
                                nullptr,      // async callback function
                                nullptr       // progress callback
                                )) {
      Serial.println("Upload successful!");
      String downloadUrl = fbdo.storageURL();
...
void setup() {
  ...
  initWiFi();
  initFirebase();
  Serial.println("Setup Firebase completed!");
...
//_____________Scan Video________________
void onButtonPress() {
  File root = SD_MMC.open("/");
  if (!root) {
      ...
      if (filename.endsWith(".avi")) {
        ...
        if (dotPos > 0) {
          ...
          if (secondDotPos > 0) {
            ...
            if (group > latest_group || (group == latest_group && number > latest_number)) {
              latest_group = group;
...
void the_camera_loop(void *pvParameter) {
  ...
  while (1) {
    ...
    } else if ((start_record == 1 && buttonPressed == 1) || (start_record == 0 && buttonPressed == 1)) {
      Serial.println("-------Scan Video ------ ");
      onButtonPress();
      last_button_state = current_button_state;
    }
  }
...
void loop() {
  ...
  if (digitalRead(12) == LOW && last_button_state == LOW) {
